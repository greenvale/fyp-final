...
local        float* avgmom_local_lh,
local        float* avgmom_local_rh,
local          int* sub_flag_local
)
{
  /* work-item variables */
  ...
  /* Set local avgmom vector to zeros ahead of accumulation */
  if (local_id == 0)
  {
    for (int i = 0; i < par.nx + 1; ++i)
    {
      avgmom_expanded_d[group_id * (par.nx + 1) + i] = 0.0;
      avgmom_local_lh[i] = 0.0;
      avgmom_local_rh[i] = 0.0;
    }
  }

  while (workgroup_flag == 0)
  {
    davgmom_lh = 0.0;
    davgmom_rh = 0.0;
    ...
    /* only move particle in loop if sub_flag == 0, i.e. sub-cycle not complete */
    if (sub_flag == 0)
    {
    ...
    /* reduction of davgmom split for all work-items */
    for (int i = 0; i < par.nx; ++i)
    {
      davgmom_lh_tmp = 0.0;
      davgmom_rh_tmp = 0.0;

      if ((sub_flag == 0) && (i == subcell))
      {
        davgmom_lh_tmp = davgmom_lh;
        davgmom_rh_tmp = davgmom_rh;
      }

      barrier(CLK_LOCAL_MEM_FENCE);
      avgmom_local_lh[i] = work_group_reduce_add(davgmom_lh_tmp);
      barrier(CLK_LOCAL_MEM_FENCE);
      avgmom_local_rh[i] = work_group_reduce_add(davgmom_rh_tmp);
      barrier(CLK_LOCAL_MEM_FENCE);
    }

    /* inform other work items if sub-cycle completed */
    sub_flag_local[local_id] = sub_flag;
    barrier(CLK_LOCAL_MEM_FENCE);

    /* decide if workgroup is completed */
    workgroup_flag = 1;
    for (int i = 0; i < local_size; ++i)
    {
      if (sub_flag_local[i] == 0)
      {
        workgroup_flag = 0;
      }
    }

    /* Add momentum contribution to global expanded array */
    if (local_id == 0)
    {
      for (int i = 0; i < par.nx; ++i)
      {
        avgmom_expanded_d[group_id * (par.nx + 1) + i]     += avgmom_local_lh[i];
        avgmom_expanded_d[group_id * (par.nx + 1) + i + 1] += avgmom_local_rh[i];
      }
    }
    barrier(CLK_LOCAL_MEM_FENCE);
  }
